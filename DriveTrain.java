...
package org.usfirst.frc5933.Sunny.subsystems;

import org.usfirst.frc5933.Sunny.RobotMap;
import org.usfirst.frc5933.Sunny.commands.*;
import edu.wpi.first.wpilibj.CANTalon;

import edu.wpi.first.wpilibj.command.Subsystem;
import src.org.usfirst.frc5933.ubot.subsystems.Joystick;
import src.org.usfirst.frc5933.ubot.subsystems.RobotDrive;
import src.org.usfirst.frc5933.ubot.subsystems.String;
import src.org.usfirst.frc5933.ubot.subsystems.StringBuilder;
import src.org.usfirst.frc5933.ubot.subsystems.TalonControlMode;
...
public class DriveTrain extends Subsystem {
    ...
    public void arcadeDrive(joystick1 stick) {
        double y = stick.getY();
        double x = stick.getX();
    ...
    private void enableBrakeMode(boolean enable) {
        leftMotor.enableBrakeMode(enable);
        rightMotor.enableBrakeMode(enable);
    ...
    private void setRampRate(int rate) {
        leftMotor.setVoltageRampRate(rate);
        rightMotor.setVoltageRampRate(rate);
    }
    ...
    private void changeControlMode(TalonControlMode mode) {
        ...
        if (mode == TalonControlMode.Position) {
            leftMotor.changeControlMode(TalonControlMode.Follower);
            leftMotor.set(12);
    ...
    public void startPositionMovement(double leftRotations, double rightRotations) {
        ...
        targetLeftPosition_ = leftMotor.getPosition() + leftRotations;
        targetRightPosition_ = rightMotor.getPosition() + rightRotations;
        ...
        if (setLeftFirst_) {
            leftMotor.set(targetLeftPosition_);
            rightMotor.set(targetRightPosition_);
        } else {
            rightMotor.set(targetRightPosition_);
            leftMotor.set(targetLeftPosition_);
        }
//        setLeftFirst_ = !setLeftFirst_;
    ...
    private void printEncoderDebugging(boolean useCount) {
        ...
        encoderDebugString_.append(leftMotor.getPosition() );
        encoderDebugString_.append("\terr:");
        encoderDebugString_.append(leftMotor.getClosedLoopError());
        encoderDebugString_.append("\ttrg:");
        encoderDebugString_.append(targetLeftPosition_);

        encoderDebugString_.append("\tR: ");
        encoderDebugString_.append("\tpos:");
        encoderDebugString_.append(rightMotor.getPosition() );
        encoderDebugString_.append("\terr:");
        encoderDebugString_.append(rightMotor.getClosedLoopError());
        encoderDebugString_.append("\ttrg:");
    ...
    public boolean hasFinishedPositionMovement() {
        double frontLeftPosition = leftMotor.getPosition();
        ...
        // May want to see if the last position is close enough to the current position 
        double frontRightPosition = rightMotor.getPosition();
    ...
    public void configForAutonomous() {
        ...
        setTalonAbsolutePosition(leftMotor);
        setTalonAbsolutePosition(rightMotor);

        setTalonFeedbackDevice(leftMotor);
        setTalonFeedbackDevice(rightMotor);

        configVoltages(leftMotor, 0, 12);
        configVoltages(rightMotor, 0, 12);

        /* set the allowable closed-loop error,
         * Closed-Loop output will be neutral within this range.
         * See Table in Section 17.2.1 for native units per rotation. 
         */
        resetClosedLoopErrors(leftMotor);
        resetClosedLoopErrors(rightMotor);

        /* set closed loop gains in slot0 */
        setClosedLoopGains(leftMotor);
        setClosedLoopGains(rightMotor);
...
//        setLeftFirst_ = !setLeftFirst_;
        ...
        // we re done.
        lastLeftPosition_ = leftMotor.getPosition();
        lastRightPosition_ = rightMotor.getPosition();
